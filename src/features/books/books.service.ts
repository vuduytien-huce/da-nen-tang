...
export const booksService = {
  ...
  normalizeIsbn(isbn: string): string {
    return isbn.replace(/^ISBN:/i, '').replace(/[-\s]/g, '').trim();
  },
  ...
  async fetchBookMetadata(isbn: string): Promise<BookMetadata | null> {
    ...
    try {
      const googleRes = await axios.get(`https://www.googleapis.com/books/v1/volumes?q=isbn:${cleanIsbn}`, { timeout: 5000 });
    ...
    try {
      const openLibRes = await axios.get(`https://openlibrary.org/api/books?bibkeys=ISBN:${cleanIsbn}&format=json&jscmd=data`, { timeout: 5000 });
    ...
    return {
      ...
      author: googleItem?.authors?.join(', ') || openLibData?.authors?.map((a: any) => a.name).join(', ') || "Unknown Author",
      description: googleItem?.description || openLibData?.notes || openLibData?.description || "",
      ...
      categories: googleItem?.categories || openLibData?.subjects?.map((s: any) => s.name) || [],
      thumbnail: openLibData?.cover?.large || openLibData?.cover?.medium || googleItem?.imageLinks?.thumbnail || googleItem?.imageLinks?.smallThumbnail || `https://covers.openlibrary.org/b/isbn/${cleanIsbn}-L.jpg`,
      isbn: cleanIsbn,
      language: googleItem?.language || "vi",
      averageRating: googleItem?.averageRating,
      ratingsCount: googleItem?.ratingsCount,
      edition: googleItem?.contentVersion || openLibData?.identifiers?.openlibrary?.[0],
      syncSource: {
        google: !!googleItem,
        openLib: !!openLibData
      }
  ...
  async enrichWithBookMetadata(
    ...
    for (const ab of audiobooks) {
      ...
      if (ab.source_platform === 'thuviensachnoi' && !matchedBooks?.length) {
  ...
  async bulkEnrichAudiobooks() {
    ...
    for (const item of enriched) {
      if (item.canonical_author || item.canonical_cover_url || item.canonical_description) {
  ...
  async searchAudiobooks(query: string, limit = 20): Promise<EnrichedAudiobook[]> {
    const { data, error } = await supabase.rpc('search_audiobooks', { query, lim: limit });
    if (error || !data) return [];
    return this.enrichWithBookMetadata(data);
  },

  async getAudiobookByISBN(isbn: string): Promise<EnrichedAudiobook | null> {
    const { data, error } = await supabase.from('audiobook_metadata').select('*').eq('isbn', isbn).maybeSingle();
  ...
  async getPersonalizedRecommendations(userId: string, limit: number = 5): Promise<Book[]> {
    try {
      ...
      if (recommendedBooks.length < limit) {
        ...
        if (!popularError && popularBooks) recommendedBooks = [...recommendedBooks, ...popularBooks];
      }
  ...
  async getSemanticRecommendations(userId: string, limit: number = 5): Promise<Book[]> {
    try {
      const { data: history, error: historyError } = await supabase
        .from('borrow_records')
      ...
      const profileText = history
        .map((h: any) => `${h.books.title} ${h.books.author} ${h.books.category}`)
        .join(' ');

      const profileEmbedding = await ai.generateEmbedding(profileText);

      const { data: recommendations, error: matchError } = await supabase.rpc('match_books', {
  ...
  async getAudiobookBySourceId(platform: string, sourceId: string): Promise<EnrichedAudiobook | null> {
    const { data, error } = await supabase
      .from('audiobook_metadata')
      .select('*')
